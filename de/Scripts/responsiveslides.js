	...
	$.fn.responsiveSlides = function(options){
		...
		_this.RS_ShowPhoto=function(next){
			...
			next.fadeIn(settings.effectTransition,function(){
				...
				if(std!=''){
					...
					$('div:eq('+$(this).index()+')',$('.pagination',_this)).css('opacity',1);
				}
		...
		$('img',$('.tgtimg',_this)).onImagesLoad(function(){
			...
			if(p.active){
				...
				$('img',$('.tgtimg',_this)).each(function(i,e){elm+='<div'+((p.dotStyle=='')?' style="float:left; height:20px; width:20px; background:url(../images/bullet.png) 0px 0px no-repeat; margin-left:2px; cursor:pointer;"':' class="'+p.dotStyle+'"')+'></div>';});
				...
				$('div',$('.pagination',_this)).mouseover(function(){
					if(p.dotStyleHover!=''){$(this).addClass(p.dotStyleHover);}else{$(this).css('background','url(../images/bullet.png) 0px -20px no-repeat');}
				}).mouseout(function(){
					if(p.dotStyleHover!=''){$(this).removeClass(p.dotStyleHover);}else{$(this).css('background','url(../images/bullet.png) 0px 0px no-repeat');}
				}).click(function(){
					_this.RS_Stop();
			...
			$('.overslide',_this).delay(settings.startDelay).fadeOut(settings.effectTransition,function(){
				...
				$('div:eq(0)',$('.pagination',_this)).css('background','url(../images/bullet.png) 0px -20px no-repeat');
			});
		});
		return _this;
    };
